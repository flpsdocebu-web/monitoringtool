...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$ready) {
    ...
    elseif (!extension_loaded('pdo_sqlite')) $error = 'PDO SQLite is not enabled for this Hostinger PHP installation.';
        ...
        try {
            $db = new PDO('sqlite:' . DB_FILE, null, null, [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION, PDO::ATTR_TIMEOUT => 15]);
            $db->exec('PRAGMA journal_mode=WAL; PRAGMA foreign_keys=ON; PRAGMA busy_timeout=15000; PRAGMA synchronous=NORMAL; PRAGMA wal_autocheckpoint=1000; CREATE TABLE users (id TEXT PRIMARY KEY, username TEXT UNIQUE NOT NULL, email TEXT NOT NULL DEFAULT "", password_hash TEXT NOT NULL, name TEXT NOT NULL, role TEXT NOT NULL, status TEXT NOT NULL, district TEXT NOT NULL DEFAULT "", school_name TEXT NOT NULL DEFAULT "", school_id TEXT NOT NULL DEFAULT "", created_at TEXT NOT NULL); CREATE TABLE drafts (user_id TEXT PRIMARY KEY, data TEXT NOT NULL, saved_at TEXT NOT NULL, FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE); CREATE TABLE reports (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, username TEXT NOT NULL, district TEXT NOT NULL, school_name TEXT NOT NULL, school_id TEXT NOT NULL, submitted_at TEXT NOT NULL, status TEXT NOT NULL, score TEXT NOT NULL, data TEXT NOT NULL, FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE); CREATE UNIQUE INDEX users_email_nocase_unique_idx ON users(email COLLATE NOCASE) WHERE email<>""; CREATE INDEX users_status_idx ON users(status); CREATE INDEX users_district_idx ON users(district); CREATE INDEX reports_user_idx ON reports(user_id); CREATE INDEX reports_date_idx ON reports(submitted_at DESC);');
