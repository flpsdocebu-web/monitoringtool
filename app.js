...
function canonicalDistrictClient(value){const key=districtKeyClient(value),options=qsa("#regDistrict option").map(x=>x.value).filter(Boolean);return options.find(name=>districtKeyClient(name)===key)||null}
