...
function automaticStatus(i){const item=indicators[i],na=qs(`#indicator_${i}_na`)?.checked,checked=selectedMovs(i).length;if(na)return"Not Applicable";if(checked===item.movs.length)return"Compliant";if(checked>=3)return"Partially Compliant";return"Not Compliant"}
...
function updateIndicatorRating(i){const status=automaticStatus(i),field=qs(`#indicator_${i}_status`),pill=qs(`#indicator_${i}_rating`),na=qs(`#indicator_${i}_na`);if(field)field.value=status;if(pill){pill.textContent=status;pill.className=`auto-rating ${status==="Compliant"?"compliant":status==="Partially Compliant"?"partial":status==="Not Applicable"?"na":"noncompliant"}`}qsa(`input[name="indicator_${i}_mov_option"]`).forEach(x=>x.disabled=Boolean(na?.checked));updateLiveScore()}
...
function buildPrintReport(){
  ...
  qs("#printReport").innerHTML=`
    ...
    <div class="print-sheet print-landscape">
      ...
      <p class="guide"><b>Automatic MOV rating:</b> All MOVs checked = Compliant; at least 3 but not all = Partially Compliant; fewer than 3 = Not Compliant. Approved Not Applicable items are excluded from scoring.</p>
