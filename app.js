...
async function renderDashboard(){
  ...
  const latestChecklist=latest.flatMap(r=>r.data?.checklist||[]),answeredItems=latestChecklist.filter(x=>x.status==="Not Applicable"||(x.movChecks?.length||0)>0).length,expectedItems=latest.reduce((n,r)=>n+(r.data?.checklist?.length||indicators.length),0),completionRate=expectedItems?answeredItems/expectedItems*100:null;
  ...
  <article class="card district-distribution-card"><div class="district-distribution-heading"><div><h3>District Submission Distribution</h3><p class="muted">Each container represents one official district. The donut shows its percentage of evaluated schools, while the details show the district average.</p></div><span class="badge gray">${new Set(latest.map(r=>canonicalDistrictClient(r.district)||r.district).filter(Boolean)).size} of 58 represented</span></div>${districtReportDonutCards(latest)}</article>
  ...
  <article class="card"><h3>Latest School Ratings by District</h3><p class="muted">Schools are grouped under one official district name. Each container shows the number of latest school submissions.</p>${districtSubmissionGroups(latest)}</article>`;
}
function districtSubmissionGroups(rows){if(!rows.length)return`<p class="muted">No submissions yet.</p>`;const groups=rows.reduce((m,r)=>{const district=canonicalDistrictClient(r.district)||(r.district||"Unassigned District").trim()||"Unassigned District";(m[district]??=[]).push({...r,district});return m},{});return`<div class="district-submission-groups">${Object.keys(groups).sort((a,b)=>a.localeCompare(b)).map(district=>{const districtRows=groups[district].sort((a,b)=>new Date(b.submittedAt)-new Date(a.submittedAt));return`<section class="district-submission-group"><header><div><h4>${esc(district)}</h4><span>Latest submitted reports from this district</span></div><strong>${districtRows.length} school submission${districtRows.length===1?"":"s"}</strong></header>${renderSubmissionTable(districtRows,false)}</section>`}).join("")}</div>`}
...
function districtKeyClient(value){return String(value||"").toLowerCase().replace(/\b(?:district|distict|distrct)\b/g," ").replace(/\bii\b/g,"2").replace(/\bi\b/g,"1").replace(/[^a-z0-9]/g,"")}
...
async function openApp(){
  ...
  buildNav();renderMeActions();fillSchoolProfile();await showPage(state.user.role==="admin"?"dashboardPage":"mePage");if(state.user.role!=="admin"&&state.user.districtValid===false)setTimeout(()=>toast("Your registered district name is not recognized. Please contact the administrator to correct it."),150);
}
(async()=>{
