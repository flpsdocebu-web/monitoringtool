...
async function renderUsers(){
  ...
  const schoolUsers=d.users.filter(u=>u.role!=="admin"),active=schoolUsers.filter(u=>u.status==="active").length,pending=schoolUsers.length-active,districts=new Set(schoolUsers.map(u=>(u.district||"Unassigned").trim().toLowerCase())).size,invalidDistricts=schoolUsers.filter(u=>u.districtValid===false).length;
