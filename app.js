...
function bindUserActions(){
  ...
  qsa(".user-reset").forEach(b=>b.onclick=()=>{openModal(`<div class="reset-password-heading"><span class="reset-password-icon">●</span><div><h2>Reset User Password</h2><p>Set a new password for <strong>${esc(b.dataset.school)}</strong>.</p></div></div><form id="adminResetPasswordForm" class="reset-password-form"><label>New Password<input id="adminNewPassword" type="password" minlength="8" maxlength="128" autocomplete="new-password" placeholder="Enter new password" required></label><label>Confirm New Password<input id="adminConfirmPassword" type="password" minlength="8" maxlength="128" autocomplete="new-password" placeholder="Re-enter new password" required></label><small>Use at least 8 characters.</small><label class="show-reset-password"><input id="showAdminResetPassword" type="checkbox"> Show passwords</label><p id="adminResetPasswordError" class="error"></p><button class="btn primary" type="submit">Update Password</button></form>`);const form=qs("#adminResetPasswordForm"),password=qs("#adminNewPassword"),confirmPassword=qs("#adminConfirmPassword");qs("#showAdminResetPassword").onchange=e=>{password.type=confirmPassword.type=e.target.checked?"text":"password"};form.onsubmit=async e=>{e.preventDefault();const error=qs("#adminResetPasswordError");error.textContent="";if(password.value!==confirmPassword.value){error.textContent="Passwords do not match.";return}const button=form.querySelector('button[type="submit"]');button.disabled=true;button.textContent="Updating…";try{await api("/users/reset-password",{method:"POST",body:JSON.stringify({id:b.dataset.id,newPassword:password.value})});closeModal();toast(`Password updated for ${b.dataset.school}.`)}catch(err){error.textContent=err.message;button.disabled=false;button.textContent="Update Password"}}});
