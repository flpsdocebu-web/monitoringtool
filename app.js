...
async function renderDashboard(){
  ...
  <article class="card district-distribution-card"><div class="district-distribution-heading"><div><h3>District Submission Distribution</h3><p class="muted">Calculated from every report listed in School Rating Submissions. Each donut shows the district's percentage of all submitted M&E Tools.</p></div><span class="badge gray">${new Set(reports.map(r=>canonicalDistrictClient(r.district)||r.district).filter(Boolean)).size} of 58 represented</span></div>${districtReportDonutCards(reports)}</article>
